...
export type CommandPaletteProps = {
  quickActions?: { name: string; icon: any; shortcut: string; url: string }[];
  recent?: { id: number; name: string; url: string }[];
...
export default function CommandPalette({
  ...
  const mode = primary
    ? "storybook-commandpalette--primary"
    : "storybook-commandpalette--secondary";
  ...
  return (
    ...
    >
      <Dialog
        as="div"
        className={`relative z-10 ${["storybook-commandpalette", mode].join(
          " "
        )}`}
        onClose={setOpen}
      >
                ...
                {query !== "" && filteredProjects.length === 0 && (
                  <div className="px-6 py-14 text-center sm:px-14">
                    ...
                    <p className="mt-4 text-sm text-gray-900">
                      We couldnt find any projects with that term. Please try
                      again.
                    </p>
                  </div>
                )}
